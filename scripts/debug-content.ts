...
async function check() {
  ...
  if (data && data.length > 0) console.log('Columns:', Object.keys(data[0]));
    ...
    if (!ws) return;
    const { error: insError } = await supabase.from('content_items').insert({ workspace_id: ws.id, title: 'Test' }).select().single();
    const { data: d2 } = await supabase.from('content_items').select('*').limit(1);
