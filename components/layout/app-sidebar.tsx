...
export function AppSidebar({ isOpen = false, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "flex h-screen flex-col border-r border-border bg-card z-50 transition-all duration-300 ease-in-out w-64",
              ...
              return (
                ...
                >
                  <div className="relative shrink-0">
                    ...
                    {badge > 0 && (
                      <span className="absolute -right-1.5 -top-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-brand text-[8px] font-bold text-white leading-none">
                        {badge > 9 ? "9+" : badge}
                      </span>
                    )}
                  </div>
